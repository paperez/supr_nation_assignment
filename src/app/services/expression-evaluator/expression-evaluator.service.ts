...
export class ExpressionEvaluatorService {
    ...
    private expressionEvalBS$: BehaviorSubject<number> = new BehaviorSubject<number>(0);
    expressionEvalObs$: Observable<number> = this.expressionEvalBS$.asObservable();

    constructor() {
    }

    getResult() {
        return this.expressionEvalBS$.getValue();
    }

    isExpressionValid(expression: string) {
        const res = this.evaluateMathExpression(expression);
        this.expressionEvalBS$.next(res);
        return res !== Constants.INVALID_NUMBER;
    }
